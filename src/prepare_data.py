...
import config
from preprocessing.utils import prepare_data
...
if __name__ == "__main__":
    ...
    parser.add_argument('-al', '--asthmoscope_locations', nargs='+',
                        help='Asthmoscope study centres used to create a dataset')
    parser.add_argument('-pl', '--pneumoscope_locations', nargs='+',
                        help='Pneumoscope study centres used to create a dataset')
    parser.add_argument('-dp', '--download_path', type=str, default='../data/raw/canonical_data/pediatric_studies',
                        help='Path to downloaded data')
    parser.add_argument('-op', '--out_path', type=str, default='../data',
                        help='Destination folder for selected recordings')
    args = parser.parse_args()

    project_locations = {
        "As": args.asthmoscope_locations if args.asthmoscope_locations is not None else ["GVA"],
        "Pn": args.pneumoscope_locations if args.pneumoscope_locations is not None else ["GVA", "POA", "DKR", "MAR",
                                                                                         "RBA", "YAO"]
    }
    ...
    _patient_df = prepare_data(_download_path, project_locations, out_path=_interim_path)
