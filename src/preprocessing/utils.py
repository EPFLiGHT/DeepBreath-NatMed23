...
from config import feat_config
...
def prepare_data(root_path, project_locations, db_columns=["patient", "diagnosis", "new_diagnosis"],
                 out_path="../data/interim", n_splits=5):
    patient_df = []
    for project, locations in project_locations.items():
        for location in locations:
            ...
            clinical_path = join(root_path, "features", f"{project}_{location}_clinical_database.csv")
            ...
            if isfile(clinical_path):
                ...
                if "new_diagnosis" not in clinical_db.columns:
                    ...
                    clinical_db["new_diagnosis"] = clinical_db.new_diagnosis.fillna(clinical_db.diagnosis)
                clinical_db = clinical_db[db_columns].dropna(subset=["patient", "new_diagnosis"])
                clinical_db = clinical_db.rename(columns={"diagnosis": "old_diagnosis",
                                                          "new_diagnosis": "diagnosis"})
                clinical_db["location"] = location

                for _, r in clinical_db.iterrows():
                    patient_id, diagnoses, old_diagnoses = r["patient"], r["diagnosis"], r["old_diagnosis"]
                    diagnoses = diagnoses_to_int(diagnoses)
                    old_diagnosis = diagnoses_to_int(old_diagnoses)[0]
                    multilabel = len(diagnoses) > 1
                    if multilabel:
                        print('\t', patient_id, "has multiple diagnoses:", r["diagnosis"])
                    ...
                    patient_folder = join(control_folder if diagnoses[0] == 0 else case_folder, patient_id)
                    ...
                    if isdir(audio_folder) and len(listdir(audio_folder)):
                        ...
                        recording_codes = sorted(
                            [f.split('.')[0] for f in listdir(audio_folder) if isfile(join(audio_folder, f))])
                    ...
                    for patient_id in patients:
                        ...
                        if isdir(audio_folder):
                            ...
                            for code in recording_codes:
                                ...
                                if (timing == 'a') and (site == 'S') and (check == '1'):  # or site == 'H'
                                    Path(out_audio_folder).mkdir(parents=True, exist_ok=True)
                                    audio_file = code_with_extension(audio_folder, code, "wav")
                                    output_file = code_with_extension(out_audio_folder, code, "wav")
                                    normalize_and_save(audio_file, output_file)
    ...
    grouped_patients = patient_df.groupby(["location", "diagnosis"])["patient"].apply(list).reset_index()
    patient_diagnosis = patient_df[["patient", "old_diagnosis", "all_diagnoses", "multilabel"]]
...
def label_matrix(labels_df, duration=10, frames_per_second=50):
    ...
    for _, r in labels_df.iterrows():
        ...
        for i in range(start_idx, end_idx):
            if i < n_frames:
                label_mat[label_idx[label], i] = 1.

    return label_mat, unique_labels


def display_spectrogram(spectrogram, title="Spectrogram", config=feat_config, duration=None, spect_path=None,
                        labels_df=None, frame_attention=None, frame_probs=None, fig_width=15, plot_height=5):
    n_plots = 1
    ...
    fig, ax = plt.subplots(nrows=n_plots, ncols=1, sharex=False, figsize=(fig_width, n_plots * plot_height))
    ...
    if frame_attention is not None:
        p_recording = round((frame_attention * frame_probs).sum() / 16, 2)  # up-sampling, divide by 16
        ...
        ax[n_plots - 3].set_ylim([0., 1.05])
        ax[n_plots - 3].set_ylabel('p')
        ax[n_plots - 3].set_title(
            "Frame Probabilities (0: healthy, 1: pathological) - Pr_recording={}".format(p_recording))
    ...
    _ = librosa.display.specshow(logspect, x_axis='time', y_axis='mel', sr=config["sr"],
                                 hop_length=config["hop_length"], fmax=config["fmax"], ax=spect_ax)
...
def display_mfcc(mfccs, config=feat_config, mfcc_path=None, figsize=(16, 9)):
    ...
    img = librosa.display.specshow(mfccs, x_axis='time', sr=config["sr"],
...
def audio_processing(features, preprocessing, audio_path, label_path, spect_path,
                     duration=None, frame_attention=None, frame_probs=None):
    if duration is not None:
        ...
        wav = feats.read_audio(audio_path, duration=30.)
    ...
    if isfile(label_path):
        labels_df = pd.read_csv(label_path, sep='\t', header=None, names=["Start", "End", "Label"], dtype=str)
        ...
        display_spectrogram(spect, title="Spectrogram", duration=duration, spect_path=spect_path,
                            labels_df=labels_df, frame_attention=frame_attention, frame_probs=frame_probs)
    else:
        display_spectrogram(spect, title="Spectrogram", duration=duration, spect_path=spect_path,
                            frame_attention=frame_attention, frame_probs=frame_probs)


def generate_spectrograms(base_folder, preprocessing=[], features=["stftw"], duration=10):
    ...
    if isdir(audio_folder):
        ...
        recording_codes = sorted([f.split('.')[0] for f in listdir(audio_folder) if isfile(join(audio_folder, f))])
        for code in recording_codes:
            ...
            audio_processing(features, preprocessing, audio_path, label_path, spect_path, duration)


def all_spectrograms(data_path, patient_df, preprocessing=[], features=["stftw"], duration=10):
...
def copy_spectrograms(folder, diagnosis_folder):
    ...
    if isdir(spect_folder):
        spectrogram_files = sorted([f for f in listdir(spect_folder) if isfile(join(spect_folder, f))])
...
def aggregate_spectrograms(data_path, patient_df, out_folder="../data/spectrograms"):
    ...
    for diagnosis_nbr in sorted(patient_df_exploded.diagnosis.unique()):
        ...
        patient_selection = patient_df_exploded[patient_df_exploded.diagnosis == diagnosis_nbr].patient.values
...
def aggregate_audios(data_path, patient_df, out_folder="../audios"):
    diagnoses = patient_df.all_diagnoses.apply(lambda ls: '+'.join([str(d) for d in ls]))
    for diagnosis in sorted(diagnoses.unique()):
        for location in patient_df.location.unique():
            ...
            diag_loc_filter = (patient_df.location == location) & (diagnoses == diagnosis)
