...
def make_sample_loader(ds, config):
    ...
    if config.balanced_sampling:
        balanced_sampler = BalancedSampler(ds.samples_df, pos_label=config.target, label_col="diagnosis",
...
def make_sample_model(samples_df, data, fold_1, fold_2, config, device):
    ...
    train_ds = AudioDataset(samples_df[train_indices], data=data[train_indices], target=config.target,
                            preprocessing=config.preprocessing, pre_config=config.pre_config, 
                            split_config=config.split_config)
    ds_1 = AudioDataset(samples_df[indices_1], data=data[indices_1], target=config.target,
                        preprocessing=config.preprocessing, pre_config=config.pre_config,
                        split_config=config.split_config, train=False)
    ds_2 = AudioDataset(samples_df[indices_2], data=data[indices_2], target=config.target,
                        preprocessing=config.preprocessing, pre_config=config.pre_config,
                        split_config=config.split_config, train=False)
...
def make_features(samples_df, data_loader, val_fold, test_fold, config, device):
    ...
    if config.save_outputs:
        ...
        # Save model outputs
        output_file = join(out_dir,
                           config.output_file.format(config.network["feature_model"], target_str, val_fold, test_fold))
        np.save(output_file, outputs)

        # Save model attention values
        attn_file = join(out_dir,
                         config.attn_file.format(config.network["feature_model"], target_str, val_fold, test_fold))
